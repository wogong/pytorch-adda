...
from core import eval, eval, train_src, train_tgt
...
class Config(object):
    ...
    model_root = os.path.expanduser(
        os.path.join('~', 'Models', 'pytorch-adda'))
    ...
    src_dataset = "svhn"
    src_encoder_restore = os.path.join(
        model_root, src_dataset + "-source-encoder-final.pt")
    ...
    tgt_dataset = "mnist"
    tgt_encoder_restore = os.path.join(
        model_root, tgt_dataset + "-target-encoder-final.pt")
    tgt_model_trained = True

    # params for setting up models
    d_model_restore = os.path.join(
        model_root, src_dataset + '-' + tgt_dataset + "-critic-final.pt")
...
if __name__ == '__main__':
    ...
    src_data_loader_eval = get_data_loader(
        params.src_dataset, dataset_root=params.dataset_root, batch_size=params.batch_size, train=False)
    ...
    tgt_data_loader_eval = get_data_loader(
        params.tgt_dataset, dataset_root=params.dataset_root, batch_size=params.batch_size, train=False)

    # load models
    src_encoder = init_model(
        net=LeNetEncoder(), restore=params.src_encoder_restore)
    src_classifier = init_model(
        net=LeNetClassifier(), restore=params.src_classifier_restore)
    tgt_encoder = init_model(
        net=LeNetEncoder(), restore=params.tgt_encoder_restore)
    ...
    if not (src_encoder.restored and src_classifier.restored and params.src_model_trained):
        src_encoder, src_classifier = train_src(
            src_encoder, src_classifier, src_data_loader, params)
    ...
    if not (tgt_encoder.restored and critic.restored and params.tgt_model_trained):
        tgt_encoder = train_tgt(src_encoder, src_classifier, tgt_encoder,
                                critic, src_data_loader, tgt_data_loader, params)
